...
async function initSotuv() {
  ...
  try {
    const [mRes, pRes, sRes] = await Promise.all([
      API.get('/mijozlar'), API.get('/mahsulotlar'), API.get('/sotuvlar')
    ]);
    
    MIJOZLAR_DATA = {};
    mRes.forEach(m => { MIJOZLAR_DATA[m.id] = { nomi: m.nomi, tur: m.turi, tel: m.tel, manzil: m.manzil, id: m.id }; });
    
    MAHSULOTLAR = pRes.map(p => ({ id: p.id, tur:'mahsulot', nomi: '📦 '+p.nomi, ombor: parseFloat(p.ombor)||0, narx: parseFloat(p.narx)||0 }));
    sotuvlar = sRes;
    
    const sel = document.getElementById('mijozSelect');
    sel.innerHTML = '<option value="">— Mijoz tanlang —</option>' + mRes.map(m=>`<option value="${m.id}">${m.nomi} (${m.turi})</option>`).join('');
    
    renderMahsulotGrid();
  } catch(e) { console.error('Sotuv data load error', e); }
}
...
function calcItem() {
  ...
  const narx = parseSum('selProdNarx');
  const jami = kg*narx;
...
function calcQarz() {
  ...
  if(tur==='aralash'){
    const naqd = parseSum('naqdSumma');
    const bank2 = parseSum('bankSumma');
...
function buildChekHTML(s) {
  ...
  return `
    ...
    <div class="chek-row bold large"><span>JAMI:</span><span>${s.summa.toLocaleString()} so'm</span></div>
    <hr class="chek-divider">
    ...
    ${s.qarz>0?`<div class="chek-row bold" style="color:var(--danger)"><span>Qarz:</span><span>${s.qarz.toLocaleString()}</span></div>`:''}
    <hr class="chek-divider">
